...
func GetUids(req map[string]string) (interface{}, error) {
	...
	err = common.Db.Select(&cronuids, "select user_id,exp_time,source,name from cronuid where del=0 and source=? and exp_time>? order by exp_time limit ?,200", req["source"], time.Now().Unix(), (p-1)*200)
	if err != nil {
		return nil, err
	}
	return cronuids, nil
